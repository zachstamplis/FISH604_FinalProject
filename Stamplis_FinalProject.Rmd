...
packages <- c("dplyr", "pls", "hyperSpec", "prospectr", "mdatools", "ggplot2", "viridis", "mgcv", "caret", "MuMIn", "gglm", "gratia", "tidyr", "ggpubr")
...
for (i in 1:10) {
  ...
  RMSE.age$lm.cal[i] <- RMSE(pred = mod$fitted.values, obs = calibrate[, 13])
  preds <- predict(mod, newdata = testing)
  RMSE.age$lm.test[i] <- RMSE(pred = preds, obs = testing[, 13])
...
for (i in 1:10) {
  ...
  preds <- predict(mod, newdata = testing)
  RMSE.age$GAM.test[i] <- caret::RMSE(pred = preds, obs = testing[, 13])
